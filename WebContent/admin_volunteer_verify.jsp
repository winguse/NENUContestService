...
if (s.user.getPermission() != User.USER_ADMINISTRATOR){
	...
	if(!Site.isEmpty(verify)){
		Person person=new Person(Integer.parseInt(request.getParameter("pid")));
		User user=new User(person.getUserBelongs());
		if("true".equals(verify)){
			person.setTitle(Person.PERSON_VOLUNTEER);
	...
	int type=Person.PERSON_VOLUNTEER_PEDDING;
	try{
	...
	case Person.PERSON_VOLUNTEER:
		title="已通过志愿者";
		break;
	case Person.PERSON_VOLUNTEER_PEDDING:
		title="待审志愿者";
		break;
	}
%>
<%@ include file="include/head_html.jspf"%>
<title>志愿者审核 - <%=title %></title>
<%@ include file="include/body_html_java.jspf"%>
<div id="main"><h1 style="text-align: center;"><%=title %></h1>
<p>
	<a href="?type=<%=Person.PERSON_VOLUNTEER%>">已通过志愿者</a>
	<a href="?type=<%=Person.PERSON_VOLUNTEER_PEDDING%>">待审志愿者</a>
</p>
<%
try{
	...
	for(Person p:psnlst){
		User user=new User(p.getUserBelongs());
		if(p.getId()==526&&!"x".equals(request.getParameter("x")))continue;		total++;
...
%>
	<tr>
		...
		<td><%=Site.htmlEncode(user.getUsername()) %></td>
		...
		<td><%=Site.htmlEncode(p.getDescription()) %></td>
		<td><%if(type==Person.PERSON_VOLUNTEER){%><a href="?pid=<%=p.getId()%>&verify=false">回归待审</a><%}else{
		%><a href="?pid=<%=p.getId()%>&verify=true">审核通过</a><%} %></td>
	</tr>
<%
	}%>
	</tbody></table>
